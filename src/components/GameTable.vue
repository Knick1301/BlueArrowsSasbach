...
<template>
  <div class="bg-white p-4 sm:p-5 rounded-xl shadow-sm border border-gray-200 h-full flex flex-col">
    <div class="flex flex-wrap items-center justify-between gap-y-3 mb-6 border-b border-gray-100 pb-4">
            ...
            <td class="pl-1.5 pr-3 sm:px-4 py-3.5">
                ...
                <div class="flex items-center gap-2">
                  <div class="min-w-0 flex-1 flex flex-col gap-1.5">
                    <div class="flex items-center gap-1.5">
                      <img v-if="game.homeLogo?.filename" loading="lazy" decoding="async" :src="game.homeLogo.filename"
                        :alt="game.hometeam" class="w-5 h-5 object-contain rounded-full shrink-0" />
                      <div v-else class="w-5 h-5 shrink-0"></div>
                      <span class="font-bold text-xs text-[#032650] leading-tight hyphens-auto [overflow-wrap:anywhere]">{{ game.hometeam }}</span>
                    </div>
                    <span class="pl-[26px] font-bold text-xs text-gray-500 leading-tight">vs.</span>
                    <div class="flex items-center gap-1.5">
                      <img v-if="game.awayLogo?.filename" loading="lazy" decoding="async" :src="game.awayLogo.filename"
                        :alt="game.awayteam" class="w-5 h-5 object-contain rounded-full shrink-0" />
                      <div v-else class="w-5 h-5 shrink-0"></div>
                      <span class="font-bold text-xs text-[#032650] leading-tight hyphens-auto [overflow-wrap:anywhere]">{{ game.awayteam }}</span>
                    </div>
                  </div>
              ...
              <div class="hidden sm:flex justify-center">
                <div class="flex items-center gap-6">
                  <img loading="lazy" decoding="async" v-if="game.homeLogo?.filename" :src="game.homeLogo.filename"
                    :alt="game.hometeam" class="w-10 h-10 object-contain rounded-full flex-shrink-0" />
                  ...
                  <div class="flex flex-col items-center text-center">
                    <div class="grid grid-cols-[1fr_auto_1fr] items-center gap-x-2">
                      <span class="font-bold text-[#032650] leading-tight text-right">{{ game.hometeam }}</span>
                      <span class="text-gray-500 text-xs font-bold leading-none">vs.</span>
                      <span class="font-bold text-[#032650] leading-tight text-left">{{ game.awayteam }}</span>
                    </div>
                    <span v-if="game.venue"
                  ...
                  <img loading="lazy" decoding="async" v-if="game.awayLogo?.filename" :src="game.awayLogo.filename"
                    :alt="game.awayteam" class="w-10 h-10 object-contain rounded-full flex-shrink-0" />
                  <div v-else class="w-10 h-10 rounded-full flex-shrink-0"></div>
                </div>
              </div>
            </td>

            <td class="hidden sm:table-cell px-1.5 sm:px-4 py-3.5 text-center font-black text-[#032650] whitespace-nowrap align-middle text-base">
              {{ game.result || '-:-' }}
            </td>
          </tr>
