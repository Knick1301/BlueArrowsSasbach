        ...
        <nav class="flex items-center bg-transparent rounded-full px-2 py-1 overflow-visible">
          ...
          <div class="hidden lg:flex items-center gap-1">
            <div v-for="item in navItems" :key="item._uid" class="relative group">
              <div
                v-editable="item"
                class="px-4 py-2 text-base font-semibold text-[#032650] hover:text-blue-700 transition-all rounded-full flex items-center gap-1 select-none cursor-default"
              >
                {{ item.title }}
                <svg
            ...
            <div v-for="item in navItems" :key="item._uid" class="border-b border-gray-100 pb-2">
              <div class="flex justify-between items-center py-2">
                <router-link
                  :to="getUrl(item.link)"
                  @click="closeMenu"
                  class="text-2xl font-bold text-[#032650]"
                >
                  {{ item.title }}
                </router-link>
                ...
                <button
                  v-if="item.subLinks?.length"
                  @click="toggleSubMenu(item._uid)"
                  class="p-2 text-[#032650]"
                >
                  <svg
