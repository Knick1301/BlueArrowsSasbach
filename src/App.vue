...
<template v-if="body.length > 0">
  <div class="min-h-screen flex flex-col font-sans bg-gray-100">
    <header
      class="sticky top-0 z-[1000] w-full h-24 3xl:h-32 bg-white shadow-md flex items-center px-4 overflow-visible">
      <div
        class="max-w-7xl 3xl:max-w-[1650px] mx-auto w-full flex items-center justify-between overflow-visible relative">
        ...
        <nav class="flex items-center bg-transparent rounded-full px-2 py-1 overflow-visible">
          ...
          <div class="hidden lg:flex items-center gap-1">
            <div v-for="item in navItems" :key="item._uid" class="relative group">
              <div v-editable="item"
                class="px-4 py-2 3xl:px-5 3xl:py-3 text-base 3xl:text-lg font-semibold text-[#032650] hover:text-blue-700 transition-all rounded-full flex items-center gap-1 select-none cursor-pointer whitespace-nowrap">
                {{ item.title }}
                <svg v-if="item.subLinks?.length" class="w-4 h-4 transition-transform group-hover:rotate-180"
                  fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              </div>

              <div v-if="item.subLinks?.length"
                ...
                <router-link v-for="sub in item.subLinks" :key="sub.title" :to="getUrl(sub.link)" @click="closeMenu"
            ...
            <router-link :to="{ name: 'kontakt' }"
              class="ml-2 bg-[#032650] text-white px-5 py-2 3xl:px-6 3xl:py-3 rounded-full text-sm 3xl:text-base font-bold hover:bg-blue-900 transition-colors shadow-sm">
              Kontakt
            </router-link>
          </div>
        </nav>
        ...
        <div class="lg:hidden flex items-center">
          <button @click="toggleMenu" class="relative w-10 h-10 text-[#032650] focus:outline-none z-[1100]"
            aria-label="Menü öffnen">
            <div class="block w-6 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <span class="block absolute h-0.5 w-6 bg-current transform transition duration-500 ease-in-out"
    ...
    <div class="bg-[#021b3d] flex justify-center items-center gap-4 3xl:gap-6 py-2 3xl:py-3 px-6">
      ...
      <span class="text-blue-200/40 text-xs 3xl:text-sm">|</span>
