...
const hauptsponsoren = computed(
  () => (story?.value?.content.hauptsponsoren as SponsorBlok[] | undefined) ?? [],
)
const sponsoren = computed(
  () => (story?.value?.content.sponsoren as SponsorBlok[] | undefined) ?? [],
)
</script>
    ...
    <div class="max-w-6xl w-[95%] mx-auto mt-15 px-4">
      ...
      >
        ...
        >
          {{
            story?.content.intro ||
            `Ohne die Unterstützung unserer Sponsoren wäre der Spielbetrieb der Blue
        ...
        <div class="border-t-2 border-gray-200 pt-8">
          ...
          <div v-if="hauptsponsoren.length" class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <a
              v-for="sponsor in hauptsponsoren"
              :key="sponsor._uid"
