    ...
    <div class="max-w-6xl w-[95%] mx-auto mt-15 px-4">
      <DecoratedCard>
        ...
        >
          {{
            story?.content.intro ||
            `Von der Gründung bis heute – die wichtigsten Meilensteine und Erfolge
        ...
        <div v-if="meisterschaften.length" class="flex flex-wrap justify-center gap-3 mb-10">
          <div
            v-for="erfolg in meisterschaften"
            :key="`banner-${erfolg._uid}`"
            class="bg-[#032650] w-30 p-1 shadow-sm"
            style="clip-path: polygon(0% 0%, 100% 0%, 100% 80%, 50% 100%, 0% 80%)"
          >
            <div
              class="h-full bg-white p-[2px]"
              style="clip-path: polygon(0% 0%, 100% 0%, 100% 80%, 50% 100%, 0% 80%)"
            >
              <div
                class="relative h-full overflow-hidden bg-[#032650] text-white text-center pt-5 pb-30 px-2 flex flex-col items-center justify-center"
                style="clip-path: polygon(0% 0%, 100% 0%, 100% 80%, 50% 100%, 0% 80%)"
              >
                <img
                  :src="basLogo"
                ...
                />
                <span
                  class="relative block text-xs font-bold uppercase tracking-wide text-yellow-100"
                >
                  {{ erfolg.jahr }}
                </span>
        ...
        <div v-if="meilensteine.length">
          ...
          <div class="md:hidden relative pl-8">
            ...
            <div class="space-y-8">
              <div v-for="eintrag in meilensteine" :key="`mobile-${eintrag._uid}`" class="relative">
                <div
                  class="absolute -left-8 top-1 w-[19px] h-[19px] rounded-full border-4 border-white shadow-sm"
                  :class="istErfolg(eintrag) ? 'bg-[#B8860B]' : 'bg-[#032650]'"
                ></div>
                <span
                  class="block text-sm font-black uppercase tracking-widest mb-1"
          ...
          <div class="hidden md:block relative">
            ...
            <div class="space-y-10">
              <div
                v-for="eintrag in meilensteine"
                :key="`desktop-${eintrag._uid}`"
              ...
              >
                ...
                <div class="relative z-10 flex justify-center py-1">
                  <div
                    class="w-[19px] h-[19px] rounded-full border-4 border-white shadow-sm"
                    :class="istErfolg(eintrag) ? 'bg-[#B8860B]' : 'bg-[#032650]'"
                  ></div>
                </div>
