...
const ladeStories = async (): Promise<GameSourceStory[]> => {
    try {
        const { data } = await storyblokApi.get('cdn/stories', { version: STORYBLOK_VERSION, per_page: 100 })
...
const teams = sortTeams([...new Set(spielTermine.flatMap((termin) => (termin.team ? [termin.team] : [])))])
...
const gefiltert = computed(() =>
    alleTermine.filter(
        (termin) =>
            ...
            (auswahl.value !== 'Heimspiele' || teamAuswahl.value === 'Alle' || termin.team === teamAuswahl.value),
    ),
)

// Laufende mehrtägige Termine (z.B. Ferienprogramm) bleiben in der Liste, die Hero-Karte zeigt den nächsten kommenden.
const hero = computed(() => gefiltert.value.find((termin) => termin.start >= heute) ?? gefiltert.value[0])
...
<template>
    <div class="mb-15">
        ...
        <div class="max-w-4xl w-[95%] mx-auto mt-15 px-4">
            ...
            <div v-if="kategorien.length > 1" class="flex flex-col gap-3 mb-8">
                <div class="flex flex-wrap justify-center gap-2">
                    <button v-for="option in ['Alle', ...kategorien]" :key="option" type="button"
                        :aria-pressed="auswahl === option" @click="auswahl = option"
                        :class="chipKlasse(auswahl === option)">
                        {{ option }}
                    </button>
                </div>

                <div v-if="auswahl === 'Heimspiele' && teams.length > 1" class="flex flex-wrap justify-center gap-2">
                    <button v-for="option in ['Alle', ...teams]" :key="option" type="button"
                        :aria-pressed="teamAuswahl === option" @click="teamAuswahl = option"
            ...
            <component :is="hero.to ? RouterLink : 'div'" v-if="hero" :to="hero.to" :class="[
                ...
                <div
                    ...
                    <img v-if="hero.logo" :src="hero.logo" alt="" decoding="async"
                ...
                <div class="md:w-3/5 p-8 lg:p-10 flex flex-col bg-[#004a87] text-white">
                    ...
                    <h2 v-if="hero.heim && hero.gast" class="mt-3 mb-3 break-words">
                        ...
                        <span class="block text-sm font-bold uppercase tracking-widest text-blue-200 my-1.5">vs.</span>
                    ...
                    <span v-if="hero.to" class="inline-flex items-center gap-1.5 text-white font-bold mt-auto">
                        Mehr erfahren
                ...
                <template v-if="gruppen.length">
                    <div v-for="(gruppe, index) in gruppen" :key="gruppe.label"
                        ...
                        <div class="flex flex-col gap-3">
                            <component :is="termin.to ? RouterLink : 'div'" v-for="termin in gruppe.eintraege"
                                :key="termin.id" :to="termin.to"
                                :class="[
                                ...
                                <div class="min-w-0 flex-1 flex flex-col gap-1">
                                    <div v-if="termin.titel !== termin.kategorie || termin.countdown"
                                        class="flex flex-wrap items-center gap-x-2 sm:gap-x-3 gap-y-1">
                                        <span v-if="termin.titel !== termin.kategorie"
                                    ...
                                    <p v-if="termin.heim && termin.gast"
                                ...
                                <img v-if="termin.logo" :src="termin.logo" alt="" loading="lazy" decoding="async"
                                    class="hidden sm:block shrink-0 sm:w-16 sm:h-16 object-contain" />

                                <span v-if="termin.to" aria-hidden="true"
                                    class="hidden sm:inline shrink-0 text-[#032650] font-bold transition-transform group-hover:translate-x-1">&rarr;</span>
                            </component>
                        </div>
                    </div>

                    <div v-if="hatMehr" class="text-center mt-8">
                        <button type="button" @click="sichtbar += SCHRITT"
