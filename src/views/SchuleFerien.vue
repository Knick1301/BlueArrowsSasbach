...
try {
  story = await useStoryblok(`skating/schuleferien`, { version: STORYBLOK_VERSION })
} catch (e) {
...
const aktuelleTermine = computed(() => {
  ...
  return [...story.value.content.dates]
    .filter(
      (termin: { ferienDatumEnde: string }) => new Date(termin.ferienDatumEnde).getTime() >= jetzt,
    )
    .sort(
      (a: { ferienDatumStart: string }, b: { ferienDatumStart: string }) =>
        new Date(a.ferienDatumStart).getTime() - new Date(b.ferienDatumStart).getTime(),
    )
})
</script>
  ...
  <div class="mb-15">
    ...
    >
      ...
      >
        ...
        >
          Die Nachwuchsförderung liegt den Blue Arrows Sasbach besonders am Herzen. Neben unserer
          regulären Laufschule bieten wir spezielle Programme für Schulen, Kindergärten und die
          schulfreie Zeit an.
        </p>
        ...
        >
          <div class="md:pr-8">
            ...
            <p class="text-gray-600 font-semibold mb-4">Schule & Kindergarten</p>
            <p class="mb-5 md:mb-10 text-base font-medium text-gray-700 leading-relaxed">
              Ob im Rahmen von Projekttagen, Sport-AGs oder als besonderes Ausflugsziel – wir
              kooperieren gerne mit lokalen Schulen und Kindergärten, um Kindern die Freude an der
              Bewegung auf Rollen zu vermitteln.
            </p>
            <ul
          ...
          <div class="mt-6 md:mt-0 md:pl-8 flex flex-col h-full">
            ...
            <p class="text-gray-600 font-semibold mb-4">Ferienspaß bei den Blue Arrows</p>
